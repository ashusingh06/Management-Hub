document.addEventListener('DOMContentLoaded', () => {
  ...
  function populateModalData(course) {
    ...
    if (notesList.length > 0) {
      if (modalNotesListStack) {
        ...
        const authBannerHtml = !loggedIn ? `
          <div class="notes-auth-banner">
            <div class="notes-auth-banner-left">
  ...
  function renderCommunityDOM(settings) {
    ...
    const grid = document.getElementById('communityChannelsGrid');
    if (!grid) return;
            ...
            <div class="community-card-top">
              <div class="community-icon-box">
                <svg viewBox="0 0 24 24" fill="currentColor">
                  <path d="M12.04 2c-5.46 0-9.91 4.45-9.91 9.91 0 1.75.46 3.45 1.32 4.95L2.05 22l5.25-1.38c1.45.79 3.08 1.21 4.74 1.21 5.46 0 9.91-4.45 9.91-9.91 0-2.65-1.03-5.14-2.9-7.01A9.816 9.816 0 0 0 12.04 2zm.01 1.67c2.2 0 4.26.86 5.82 2.41a8.17 8.17 0 0 1 2.4 5.83c0 4.54-3.7 8.24-8.24 8.24-1.48 0-2.93-.4-4.2-1.15l-.3-.18-3.12.82.83-3.04-.2-.31a8.21 8.21 0 0 1-1.26-4.38c0-4.54 3.7-8.24 8.24-8.24h.03zm-3.56 4.39c-.2 0-.52.07-.79.37-.27.3-1.04 1.02-1.04 2.48 0 1.46 1.06 2.87 1.21 3.07.15.2 2.09 3.19 5.06 4.47.71.3 1.26.49 1.69.62.71.23 1.36.19 1.87.12.57-.09 1.76-.72 2.01-1.42.25-.7.25-1.3.17-1.42-.07-.13-.27-.2-.57-.35s-1.76-.87-2.03-.97c-.27-.1-.47-.15-.67.15-.2.3-.77.97-.94 1.17-.17.2-.35.22-.64.08-.3-.15-1.25-.46-2.39-1.47-.88-.79-1.48-1.76-1.65-2.06-.17-.3-.02-.46.13-.61.13-.13.3-.35.45-.52.15-.17.2-.3.3-.5.1-.2.05-.37-.02-.52-.08-.15-.67-1.62-.92-2.22-.24-.58-.49-.5-.67-.51l-.57-.01z"/>
                </svg>
              </div>
              <span class="community-badge-pill">
                <span class="community-badge-dot"></span>
                <span class="community-badge-label">${wa.badge || 'Active Community'}</span>
              </span>
            </div>
            <div class="community-card-body" style="margin-top: 18px;">
              <h3>${wa.title || 'Official WhatsApp Community'}</h3>
              <p>${wa.subtitle || 'Active discussion group for IITM BS students. Ask questions, share notes, and get instant updates.'}</p>
            </div>
          </div>
          <div class="community-card-footer">
            <a href="${wa.url || 'https://chat.whatsapp.com/ClB7IBHguCWKgjWtuyjfvf'}" target="_blank" rel="noopener noreferrer" class="btn-community-join">
              <span>${wa.buttonText || 'Join WhatsApp Community'}</span>
        ...
        <div class="community-card card-telegram">
          <div>
            <div class="community-card-top">
              ...
              <span class="community-badge-pill">
                <span class="community-badge-dot"></span>
                <span class="community-badge-label">${tg.badge || 'Fast Alerts & Resources'}</span>
              </span>
            </div>
            <div class="community-card-body" style="margin-top: 18px;">
              <h3>${tg.title || 'Telegram Channel & Group'}</h3>
              <p>${tg.subtitle || 'Direct broadcast channel & discussion forum for study materials, PYQ papers, important deadlines, and academic announcements.'}</p>
            </div>
          </div>
          <div class="community-card-footer">
    ...
    // Custom Extra Channels
    if (Array.isArray(settings.customChannels)) {
      settings.customChannels.forEach(ch => {
        if (ch && ch.enabled !== false && ch.url) {
          cardsHtml.push(`
            <div class="community-card card-custom">
              <div>
                <div class="community-card-top">
                  ...
                  <span class="community-badge-pill">
                    <span class="community-badge-dot"></span>
                    <span class="community-badge-label">${ch.badge || 'Community Channel'}</span>
                  </span>
                </div>
                <div class="community-card-body" style="margin-top: 18px;">
                  <h3>${ch.title || 'Community Channel'}</h3>
                  <p>${ch.subtitle || 'Connect with the student network and peer group.'}</p>
                </div>
              </div>
              <div class="community-card-footer">
                <a href="${ch.url}" target="_blank" rel="noopener noreferrer" class="btn-community-join">
                  <span>${ch.buttonText || 'Join Channel'}</span>
                  <svg class="btn-arrow-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M7 17 17 7"/><path d="M7 7h10v10"/></svg>
                </a>
              </div>
            </div>
  ...
  async function loadAndRenderCommunity() {
    ...
    try {
      const cached = localStorage.getItem('mghub_community');
      if (cached !== null) {
        const parsed = JSON.parse(cached);
        if (parsed && typeof parsed === 'object') {
          settings = { ...DEFAULT_COMMUNITY_SETTINGS, ...parsed };
          renderCommunityDOM(settings);
        }
      }
    } catch (e) {}

    // Cloud Firestore Sync
    if (typeof fetchCommunitySettingsFromFirestore === 'function') {
      try {
        const cloudData = await fetchCommunitySettingsFromFirestore();
        if (cloudData && typeof cloudData === 'object') {
          settings = { ...DEFAULT_COMMUNITY_SETTINGS, ...cloudData };
          try { localStorage.setItem('mghub_community', JSON.stringify(settings)); } catch (e) {}
          renderCommunityDOM(settings);
          return;
        }
      } catch (e) {}
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (apiData && typeof apiData === 'object') {
          settings = { ...DEFAULT_COMMUNITY_SETTINGS, ...apiData };
          try { localStorage.setItem('mghub_community', JSON.stringify(settings)); } catch (e) {}
          renderCommunityDOM(settings);
          return;
        }
  ...
  if (typeof listenToCommunitySettingsFromFirestore === 'function') {
    listenToCommunitySettingsFromFirestore((cloudCommunity) => {
      if (cloudCommunity) {
        const updated = { ...DEFAULT_COMMUNITY_SETTINGS, ...cloudCommunity };
        try { localStorage.setItem('mghub_community', JSON.stringify(updated)); } catch (e) {}
        renderCommunityDOM(updated);
      }
    });
  }
});
